...
real(8)::a1,a2,a3,a4,tmin,kh,aj,sum1,s0,s1,s2,s3,s4

   a1=minval(xs)+0.001
   a3=maxval(xs)
   a2=(a1+a3)/2.0
   
   call GSSLQP(a1,a2,a3,xs,y,n,hbw) 
   !write(401,*)hbw
...
*((2.0*3.141592653)**(1.0/2.0)*hbw**1.0)**(-1.0)   
      ...
      s3=s3+(xs(c2)-xs(c1))**3.0*kh
      s4=s4+(xs(c2)-xs(c1))**4.0*kh
   end do
   s0=s0/n
   s1=s1/n
   s2=s2/n
...
num=num+((s2*s4-s3*s3)-(s1*s4-s2*s3)*(xs(c2)-xs(c1))+(s1*s3-s2*s2)*(xs(c2)-xs(c1))**2.0)*y(c2)*aj/0.0000001
        else
num=num+((s2*s4-s3*s3)-(s1*s4-s2*s3)*(xs(c2)-xs(c1))+(s1*s3-s2*s2)*(xs(c2)-xs(c1))**2.0)*y(c2)*aj&
...
!Estimation of PMI
subroutine PMIRELQP(inx,outy,sn,ni)
implicit none
...
do c2=1,ni
   ...
   do c3=1,ni
      if (inx(1,c3)/=-9999.0) then
          call LQP(sinx,inx(1:sn,c3),sn,gin)
          do c1=1,sn
