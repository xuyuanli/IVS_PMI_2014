...
!*************GRNN prediction with GRNN*********************
subroutine LLP(xs,y,n,xg)
implicit none
...
real(8)::a1,a2,a3,a4,tmin,kh,aj,sum1,s0,s1,s2

   a1=minval(xs)+0.001
   a3=maxval(xs)
   a2=(a1+a3)/2.0
   
   call GSSLLE(a1,a2,a3,xs,y,n,hbw) 
   !write(401,*)hbw
...
do c1=1,n    
   ...
   s2=0.0
   do c2=1,n
      kh=dexp(-(xs(c2)-xs(c1))**2.0/2.0/hbw**2.0)&
*((2.0*3.141592653)**(1.0/2.0)*hbw**1.0)**(-1.0)   
      s0=s0+kh 
      s1=s1+(xs(c2)-xs(c1))*kh
      s2=s2+(xs(c2)-xs(c1))**2.0*kh
   end do
   s0=s0/n
   s1=s1/n
   s2=s2/n
   ...
   do c2=1,n
      if (c2==c1) then
            ...
            sum1=(xs(c2)-xs(c1))**2.0/2.0/hbw**2.0
        aj=dexp(-sum1)*((2.0*3.141592653)**(1.0/2.0)*hbw**1.0)**(-1.0)
        if ((s2*s0-s1**2.0)<0.0000001) then
            num=num+(s2-s1*(xs(c2)-xs(c1)))*y(c2)*aj/0.0000001
        else
            num=num+(s2-s1*(xs(c2)-xs(c1)))*y(c2)*aj/(s2*s0-s1**2.0)
        end if
      end if
   end do       
   
   if (num<=1.0*10.0**(-6.0)) then
       xg(c1)=1.0*10.0**(-6.0)/real(n)
   else  
       xg(c1)=num/real(n)  
   end if
...
!Estimation of PMI
subroutine PMIRKLLP(inx,outy,sn,ni)
implicit none
...
do c2=1,ni
   ...
   do c3=1,ni
      if (inx(1,c3)/=-9999.0) then
          call LLP(sinx,inx(1:sn,c3),sn,gin)
          do c1=1,sn
