...
!**********************************MLP ANN***************************************
subroutine MLPANN(inx,outy,nr,nc,outyest)
implicit none

real(8),intent(in)::inx(:),outy(:)
real(8),intent(out)::outyest(:)
integer(8),intent(in)::nr,nc
integer(8)::c1,c2,c3   !counters
real(8),allocatable::w0(:),w1(:),w2(:),w3(:)
real(8),allocatable::outyestp(:,:),AICv(:)
real(8)::mins,maxs,vars,kurts,rmses,mares,ces,ioads,pis,as,ss,& 
mces,mioads,mpis,minAIC !Performance criteria

allocate(w0(nc+1))
allocate(w1(nc+3))
allocate(w2(2*nc+5))
allocate(w3(3*nc+7))
allocate(outyestp(nr,4))
allocate(AICv(4))
...
!MLP-ANN 0 HN
call PSOA0(inx,outy,nr,nc,w0)    !Find the global optimal weights with PSO
call ANN0HN(inx,outy,nr,nc,w0,outyestp(1:nr,1),AICv(1))  !Find the corresponding estimated output

!MLP-ANN 1 HN
call PSOA1(inx,outy,nr,nc,w1)    !Find the global optimal weights with PSO
call ANN1HN(inx,outy,nr,nc,w1,outyestp(1:nr,2),AICv(2))  !Find the corresponding estimated output

!MLP-ANN 2 HN
call PSOA2(inx,outy,nr,nc,w2)    !Find the global optimal weights with PSO
call ANN2HN(inx,outy,nr,nc,w2,outyestp(1:nr,3),AICv(3))  !Find the corresponding estimated output

!MLP-ANN 3 HN
call PSOA3(inx,outy,nr,nc,w3)    !Find the global optimal weights with PSO
call ANN3HN(inx,outy,nr,nc,w3,outyestp(1:nr,4),AICv(4))  !Find the corresponding estimated output

minAIC=minval(AICv)

do c3=1,4
   if (AICv(c3)==minAIC) then
       do c1=1,nr
          outyest(c1)=outyestp(c1,c3)
       end do
   end if       
end do
...
!Estimation of PMI
subroutine PMIREANN(inx,outy,sn,ni)
implicit none
...
do c2=1,ni
   ...
   do c3=1,ni
      if (inx(1,c3)/=-9999.0) then
          call MLPANN(sinx,inx(1:sn,c3),sn,1_8,gin)
          do c1=1,sn
   ...
   call MLPANN(sinx,outy,sn,1_8,gout)   
...
write(203,*)
write(203,'(A7)')"Summary"
write(203,'(3A9)')"NO","ORD","PMI"
do c2=1,ni
   write(203,'(I2,X,I2,X,F6.3)')c2, ord(c2),pmi(c2)
end do

write(203,'(A8)')"By AICp:"
minAIC=minval(AIC1)
do c2=1,ni
   if (AIC1(c2)==minAIC) then
       write(203,'(A7,I2,A24)')"Input (",ord(c2),") is the turnning point."
       c4=c2
   end if
end do  
...
!write(303,'(I2,X,25(I2,X),25(F6.3,X))')c4,(ord(c2),c2=1,ni),(pmi(c2),c2=1,ni) !Adjust ord & PMI format for each problem
write(303,*)c4,(ord(c2),c2=1,ni),(pmi(c2),c2=1,ni) !Adjust ord & PMI format for each problem
