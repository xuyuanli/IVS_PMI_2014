...
real(8)::sum1,sum2,dets,cv,hc,kcv,kx !sum(K(.)),bandwidth,MPDF,determinant
integer(8)::i,j,k,c(4) !Counter,size,dim
real(8)::hxy,xl,xu,yl,yu,a(4),hr
integer(4)::ok
real(8),allocatable::kdex(:),kdey(:),kdexy(:,:)

allocate(kdex(100))
allocate(kdey(100))
allocate(kdexy(100,100))
...
do i=1,100
   kdex(i)=xl+(xu-xl)*(i-1)/99
   kdey(i)=yl+(yu-yl)*(i-1)/99
end do   
...
ok=Rput("yu",yu)
ok=Rput("hr",hr)
ok=Reval("library(sparr)")
ok=Reval("a<-ppp(x,y,window=owin(c(xl,xu),c(yl,yu)))")
ok=Reval("a.adaptive.density <- bivariate.density(data=a,ID=NULL,pilotH=hr,edgeCorrect=TRUE,res=100)") !
!ok=Reval("plot(a.adaptive.density, display = '3d', col = heat.colors(20), aspect = 1:2)")
ok=Rget("a.adaptive.density$zSpec",fi)
ok=Rclose()
