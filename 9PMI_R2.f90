...
integer(8)::c1,c2,c3
real(8)::num,den,aj,bw,lg,ug,ag,cv,hc,kx

lg=minval(xs)+0.001
ug=maxval(xs)
ag=(lg+ug)/2.0

call GSSBK(lg,ag,ug,xs,y,n,bw)
...
!Estimation of PMI
subroutine PMIREBK(inx,outy,sn,ni)
implicit none
...
do c2=1,ni
   ...
   do c3=1,ni
      if (inx(1,c3)/=-9999.0) then
          call BK(sinx,inx(1:sn,c3),sn,gin)
          do c1=1,sn
