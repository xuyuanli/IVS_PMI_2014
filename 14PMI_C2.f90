...
subroutine LBR(xs,y,n,xg)
implicit none
real(8),intent(in)::xs(:),y(:)
integer(8),intent(in)::n
real(8),intent(out)::xg(:)
real(8)::num,den,hbw,sdv,hbwt
integer(8)::c1,c2,c3,c4
real(8)::a1,a2,a3,a4,tmin,kh,aj

   a1=minval(xs)+0.001
   a3=maxval(xs)
   a2=(a1+a3)/2.0
   
   call GSSSP(a1,a2,a3,xs,y,n,hbwt) 
   !write(401,*)hbw
...
do c1=1,n    
   ...
   do c2=1,n
      if (c2==c1) then
            num=num
            den=den
         else if (xs(c2)<=hbwt) then
              hbw=max((xs(c2)-a4),0.000001) 
              aj=dexp(-(xs(c2)-xs(c1))**2.0/2.0/hbw**2.0)
              num=num+y(c2)*aj
              den=den+aj
         else
              hbw=hbwt
...
!Estimation of PMI
subroutine PMIRKLBR(inx,outy,sn,ni)
implicit none
...
do c2=1,ni
   ...
   do c3=1,ni
      if (inx(1,c3)/=-9999.0) then
          call LBR(sinx,inx(1:sn,c3),sn,gin)
          do c1=1,sn
